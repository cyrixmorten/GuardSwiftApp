...
//            if (tasksCache.getArrivedWithCheckpoints(ParseTask.TASK_TYPE.REGULAR).isEmpty()) {
...
//        TaskCache tasksCache = GuardSwiftApplication.getInstance().getCacheFactory().getTaskCache();
...
//            for (ParseTask task: tasksCache.getArrived()) {
//                boolean clientHasCheckpoints = task.getClient().hasCheckPoints();
//                boolean taskIsRegularType = task.getTaskType() == ParseTask.TASK_TYPE.REGULAR;
...
//        Set<ParseTask> arrivedTasks = tasksCache.getArrivedWithCheckpoints(ParseTask.TASK_TYPE.REGULAR);
